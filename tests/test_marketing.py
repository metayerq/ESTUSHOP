...
# ── La dépense des campagnes ─────────────────────────────────────────────────────────────────
#
# ⚠️ L'ANALYSE D'EFFET (venues avant/après l'envoi) A ÉTÉ RETIRÉE LE 20/09/2026, à la demande de
# Quentin. Ce qui reste est MESURÉ : le coût vient des segments réellement facturés, et ne
...
def test_le_cout_vient_des_segments_factures(client, admin, monkeypatch):
    monkeypatch.setattr(flask_app, "_supa_get", lambda t, p: [CAMP])
    ...
    assert d["campagnes"][0]["cout_centimes"] == 2 * flask_app.PRIX_SEGMENT_CENTIMES


def test_la_depense_survit_a_une_table_absente(client, admin, monkeypatch):
    def absent(*a, **k):
        raise flask_app.SupabaseSchemaError("card_campaigns n'existe pas")
...
def test_lecran_dit_que_ce_nest_pas_toute_la_facture():
    ...
    assert "pas toute la facture" in html[i:i + 900]


def test_lecran_ne_promet_plus_deffet():
    """
    ⚠️ LA FONCTIONNALITÉ A ÉTÉ RETIRÉE, PAS MASQUÉE. Un reste de libellé — « ce qui s'est passé
    ensuite », « venus » — ferait chercher un écran qui n'existe plus.
    """
    html = _gabarit()
    for reste in ("ce qui s'est pass", "clients venus", "EN COURS", "réveill"):
        assert reste not in html, f"reste de l'analyse d'effet dans le gabarit : {reste}"
